...
export async function POST(req: Request) {
  ...
  let points: string[];
  try { points = await extractAtomicObservations(parsed.data.raw); }
  ...
  const { data, error } = await supabase.from("chatthoughts_observation_thoughts")
    .insert({ ...parsed.data, points, added_point_indexes: comparison.non_conflicting_point_indexes, status: comparison.conflicts.length ? "pending" : "resolved" }).select("*").single();
